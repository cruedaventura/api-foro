...
public class StingDAOQuery
{
    ...
    //public final static String GET_STING_BY_GROUPID = "select hex(s.id) as id, hex(s.userid) as userid, hex(groupid) as groupid, s.content, s.subject, s.creation_timestamp, s.last_modified, u.fullname from stings s, users u where groupid=unhex(?) and u.id=s.userid";
